...
public class NotaProvaTest {
	...
	public void questaoUmTest() {
		...
		try {
			final var p1 = new ProdutoModel("P1", BigDecimal.ONE, EnumTipoProdutoModel.MEDICAMENTO);
			final var p2 = new ProdutoModel("P2", BigDecimal.TEN, EnumTipoProdutoModel.NAO_MEDICAMENTO);

			assertEquals(1L, p1.getId());
			assertEquals("P1", p1.getNome());
			assertEquals(BigDecimal.ONE, p1.getValor());
			assertEquals(EnumTipoProdutoModel.MEDICAMENTO, p1.getTipo());

			assertEquals(2L, p2.getId());
			assertEquals("P2", p2.getNome());
			assertEquals(BigDecimal.TEN, p2.getValor());
			assertEquals(EnumTipoProdutoModel.NAO_MEDICAMENTO, p2.getTipo());
		...
		try {
			...
			assertEquals(EnumStatusPedidoModel.AGUARDANDO_PAGAMENTO, pedido.getStatus());

			pedido.finalizar();

			assertNotNull(pedido.getDataPagamento());
			assertTrue(pedido.getDataPagamento() instanceof LocalDateTime);
			assertEquals(EnumStatusPedidoModel.PAGO, pedido.getStatus());
	...
	public void questaoTresTest() {
		...
		try {
			...
			assertEquals(EnumTipoProdutoModel.MEDICAMENTO, p.getTipo());
			id = p.getId();
			...
			assertEquals(EnumTipoProdutoModel.NAO_MEDICAMENTO, p.getTipo());
		...
		try {
			...
			assertEquals(EnumTipoProdutoModel.NAO_MEDICAMENTO, p[0].getTipo());
			nq = nq.add(BigDecimal.valueOf(0.25));
		} catch (Exception e) {
			e.getMessage();
			fail();
	...
	public void questaoQuatroTest() {
		...
		try {
			...
			assertEquals(EnumStatusPedidoModel.AGUARDANDO_PAGAMENTO, p.getStatus());
			id1 = p.getId();
		...
		try {
			...
			assertEquals(EnumStatusPedidoModel.AGUARDANDO_PAGAMENTO, pe1.getStatus());
			...
			assertEquals(EnumStatusPedidoModel.AGUARDANDO_PAGAMENTO, pe2.getStatus());
		...
		try {
			...
			assertEquals(id2, p.getId());
			assertEquals(1, p.getItens().size());
			assertEquals(1, BigDecimal.valueOf(9).compareTo(p.getValorTotal()));
			assertNotNull(p.getDataPedido());
			assertNotNull(p.getDataPagamento());
			assertTrue(p.getDataPagamento() instanceof LocalDateTime);
			assertEquals(EnumStatusPedidoModel.PAGO, p.getStatus());
		...
		try {
            ...
            assertEquals(EnumStatusPedidoModel.PAGO, pedidos[0].getStatus());
	...
	public void questaoCincoTest() {
		...
		try {
			...
			final FarmaceuticoModel f = this.rest.getForObject(getURL("/pessoa/cadastrar-farmaceutico" + param),
					FarmaceuticoModel.class);
		...
		try {
			...
			final ClienteModel c = this.rest.getForObject(getURL("/pessoa/cadastrar-cliente" + param),
					ClienteModel.class);
